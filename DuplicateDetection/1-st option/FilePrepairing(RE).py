...
df1["row"]=df1["row"].apply(lambda x: re.sub('http\w*','', x))#Убирает все слова, начинающиеся с http (ссылки)
df1["row"]=df1["row"].apply(lambda x: re.sub('fujitsucom','', x))#Убирает все слова, начинающиеся с http (ссылки)
def clean_text(text):
    text = re.sub(r"\n", " ", text)
    return text
